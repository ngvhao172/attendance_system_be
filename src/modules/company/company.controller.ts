...
export class CompanyController {
  ...
  async create(
    ...
    return new SuccessResponse({ data: null, message: "company created" }).send(
      res,
    );
  }
  ...
  async calculateSalary(
    ...
    const { user } = req;
    const salary = this.companyService.calculateSalary(
      user.id,
      updateCompanyDto.employeeId,
      updateCompanyDto.month,
      updateCompanyDto.year,
    );
    return new SuccessResponse({
      data: salary,
      message: `Salary of employee in ${updateCompanyDto.month} ${updateCompanyDto.year}`,
    }).send(res);
  }
}
