import { Controller, Get, Post, Body, Param, Put, Delete, Req, UseGuards, Res } from '@nestjs/common';
...
export class EmployeeController {
  constructor(private readonly employeeService: EmployeeService, private readonly companyService: CompanyService) {}

  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles([ERole.Company])
  @Get()
  async findAll(@Req() req: RequestHasUserDTO & Request, @Res() res: Response): Promise<Employee[]> {
    const {user} = req;
    ...
    return new SuccessResponse({data: employees, message: "Get employees"}).send(res);
  }
