...
export class CreateEmployeeAndCompanyTables1735011400615 implements MigrationInterface {
    name = 'CreateEmployeeAndCompanyTables1735011400615'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`company\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`name\` varchar(255) NOT NULL, \`address\` varchar(255) NOT NULL, \`userId\` int NULL, UNIQUE INDEX \`REL_c41a1d36702f2cd0403ce58d33\` (\`userId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`employee\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`name\` varchar(255) NOT NULL, \`position\` varchar(255) NOT NULL, \`baseWage\` decimal NOT NULL, \`userId\` int NULL, \`companyId\` int NULL, UNIQUE INDEX \`REL_f4b0d329c4a3cf79ffe9d56504\` (\`userId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
